    ...
    try {
      const flatData = surveys.map((row) => {
        ...
        if (row.role !== 'Dokter Praktik Mandiri') {
          ...
          base["PRB_Total Peserta"] = row.prb?.jumlah || row.prb?.totalPeserta || '-';
          base["PRB_Kunjungan Rutin"] = row.prb?.rutinKunjungan || row.prb?.kunjunganRutin || '-';
          base["PRB_Tidak Berkunjung"] = row.prb?.tidakBerkunjung || '-';
          base["PRB_Kasus_DM"] = row.prb?.peserta_dm || row.prb?.kasus?.DM || '-';
          base["PRB_Kasus_Hipertensi"] = row.prb?.peserta_ht || row.prb?.kasus?.Hipertensi || '-';
          base["PRB_Kasus_Jantung"] = row.prb?.peserta_jantung || row.prb?.kasus?.Jantung || '-';
          base["PRB_Kasus_PPOK"] = row.prb?.peserta_ppok || row.prb?.kasus?.PPOK || '-';
          base["PRB_Kasus_Asma"] = row.prb?.peserta_asma || row.prb?.kasus?.Asma || '-';
          base["PRB_Kasus_Stroke"] = row.prb?.peserta_stroke || row.prb?.kasus?.Stroke || '-';
          base["PRB_Kasus_Epilepsi"] = row.prb?.peserta_epilepsi || row.prb?.kasus?.Epilepsi || '-';
          base["PRB_Kasus_Skizofrenia"] = row.prb?.peserta_skizofrenia || row.prb?.kasus?.Skizofrenia || '-';
          base["PRB_Kasus_SLE"] = row.prb?.peserta_sle || row.prb?.kasus?.SLE || '-';
