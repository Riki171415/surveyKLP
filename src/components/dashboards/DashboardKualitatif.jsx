    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {!isPrinting && (
        <div className="w-full">
          ...
          {/* Analisis Persentase Keluhan W8 */}
          {(() => {
            const w8Data = rawData.filter(item => item.question.includes('[W8]'));
            if (w8Data.length === 0) return null;
            
            const w8Categories = {
              'Sistem P-Care / IT': { keywords: ['pcare', 'p-care', 'sistem', 'internet', 'jaringan', 'error', 'server', 'simpus', 'lemot', 'aplikasi', 'bridging', 'koneksi'], count: 0 },
              'Beban Kerja & SDM': { keywords: ['beban', 'sdm', 'tenaga', 'kurang', 'sibuk', 'waktu', 'lelah', 'dokter', 'petugas', 'perawat', 'kapasitas', 'pasien banyak'], count: 0 },
              'Sistem Rujukan / RS': { keywords: ['rujukan', 'bpjs', 'rs', 'fktp', 'lanjutan', 'rumah sakit', 'menolak', 'kuota', 'spesialis', 'sistem rujukan', 'zonasi'], count: 0 },
              'Obat / Farmasi (PRB)': { keywords: ['obat', 'farmasi', 'stok', 'apotek', 'resep', 'kosong', 'formularium', 'prb', 'ketersediaan'], count: 0 },
              'Pembiayaan / Kapitasi': { keywords: ['kapitasi', 'biaya', 'dana', 'uang', 'klaim', 'bayar', 'insentif', 'pembiayaan', 'anggaran', 'jasa'], count: 0 },
              'Sarana & Prasarana': { keywords: ['sarana', 'prasarana', 'alat', 'alkes', 'usg', 'reagen', 'gedung', 'fasilitas', 'ruangan'], count: 0 }
            };

            w8Data.forEach(item => {
              const text = item.answer.toLowerCase();
              Object.keys(w8Categories).forEach(cat => {
                if (w8Categories[cat].keywords.some(kw => text.includes(kw))) {
                  w8Categories[cat].count++;
                }
              });
            });

            const w8ChartData = Object.keys(w8Categories)
              .map(cat => ({
                name: cat,
                value: w8Categories[cat].count,
                percent: ((w8Categories[cat].count / w8Data.length) * 100).toFixed(1)
              }))
              .filter(d => d.value > 0)
              .sort((a, b) => b.value - a.value);

            return (
              <div className="bg-white p-8 rounded-2xl border border-rose-100 shadow-sm relative overflow-hidden mb-8">
                <div className="absolute top-0 right-0 p-6 opacity-5">
                  <BarChart2 className="w-48 h-48 text-rose-600" />
                </div>
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-slate-800">Persentase Keluhan Wawancara (W8)</h2>
                      <p className="text-sm text-slate-500">Persentase otomatis dari ekstraksi jawaban {w8Data.length} responden.</p>
                    </div>
                  </div>
                  
                  <div className="h-[320px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={w8ChartData} layout="vertical" margin={{ top: 10, right: 40, left: 20, bottom: 0 }}>
                        ...
                        <RechartsTooltip cursor={{ fill: '#F1F5F9' }} formatter={(value, name, props) => [`${value} Responden (${props.payload.percent}%)`, 'Mengeluhkan']} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                        <Bar dataKey="value" fill="#f43f5e" radius={[0, 6, 6, 0]} barSize={32}>
                          <LabelList dataKey="percent" position="right" formatter={(val) => `${val}%`} fill="#475569" fontSize={13} fontWeight={600} />
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                  <p className="text-xs text-slate-400 mt-4 text-center italic">*Satu responden dapat menyampaikan lebih dari satu jenis keluhan, sehingga total persentase dapat melebihi 100%.</p>
                </div>
              </div>
            );
          })()}
