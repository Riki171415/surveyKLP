...
for i in range(10):
    sheet_prb.write(f'A{27+i}', f'Penyakit {i}')
    sheet_prb.write_number(f'B{27+i}', 0)
...
sheet_hc.write('A5', 'Rata-rata Kunjungan per Bulan', header_format); sheet_hc.write_number('B5', 0)

sheet_hc.write('A9', 'Kondisi Pasien', header_format); sheet_hc.write('B9', 'Jumlah', header_format)
for i in range(6):
    sheet_hc.write(f'A{10+i}', '')
    sheet_hc.write_number(f'B{10+i}', 0)
...
for i in range(6):
    sheet_pal.write(f'A{9+i}', '')
    sheet_pal.write_number(f'B{9+i}', 0)
