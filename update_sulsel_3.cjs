...
// Delete the incorrect root-level 'Sulawesi Selatan' added by previous run
if (mapping['Sulawesi Selatan']) {
  delete mapping['Sulawesi Selatan'];
}

if (!mapping.fktp) mapping.fktp = {};
if (!mapping.dpm) mapping.dpm = {};

if (!mapping.fktp['Sulawesi Selatan']) mapping.fktp['Sulawesi Selatan'] = {};
if (!mapping.dpm['Sulawesi Selatan']) mapping.dpm['Sulawesi Selatan'] = {};
...
for (let i = 1; i < puskCsv.length; i++) {
  ...
  if (rawText.toLowerCase().includes('sulawesi selatan') || rawText.toLowerCase().includes('sulsel')) {
    ...
    let list = getFaskesList('Sulawesi Selatan', matchedKab, 'fktp');
...
for (let i = 1; i < fasCsv.length; i++) {
  ...
  if (normalizeStr(prov) === 'sulawesiselatan') {
    ...
    if (jenis === 'Klinik') {
      ...
      let list = getFaskesList(prov, kab, 'fktp');
      ...
      let list = getFaskesList(prov, kab, 'dpm');
...
console.log(`Success! Added ${matchCount} puskesmas, ${kMatch} Klinik, and ${dpmMatch} DPM from Sulawesi Selatan.`);
