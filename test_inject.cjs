...
const addFaskesStrictLog = (pProv, pKab, pNama, type) => {
  if (pKab === 'Kabupaten Bantul' && type === 'puskesmas') {
    console.log('Adding', pNama, 'to', pProv, pKab);
  }
  return addFaskesStrict(pProv, pKab, pNama, type);
};
...
if (fs.existsSync(puskesmasFile)) {
  ...
  for (let i = 1; i < puskesmasLines.length; i++) {
    ...
    if (parts.length >= 4) {
      ...
      if (prov && kab !== "Lainnya") {
        addFaskesStrictLog(prov, kab, namaPuskesmas, 'puskesmas');
      }
    }
  }
}
