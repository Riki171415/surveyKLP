...
const rightPanelStart = code.indexOf('{/* ══════════ PANEL DETAIL KANAN ══════════ */}');
const scrollableStart = code.indexOf('<div className="flex-1 overflow-y-auto', rightPanelStart);

const footerAksiIndex = code.indexOf('{/* Footer aksi */}');
let innerJSX = '';

if (footerAksiIndex !== -1) {
  // Just take everything up to the footer aksi comment
  innerJSX = code.substring(scrollableStart, footerAksiIndex);
  
  // innerJSX currently opens `<div className="flex-1 overflow-y-auto...">`
  // But since we cut off before the closing `</div>` of that container (which is after footerAksi),
  // we must manually add the closing `</div>` for it.
  innerJSX += '</div>';
} else {
  const scrollableEnd = code.lastIndexOf(')}');
  innerJSX = code.substring(scrollableStart, scrollableEnd);
}
